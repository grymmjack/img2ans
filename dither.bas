' core/dither.bas — palette-based dithering (ordered & Floyd–Steinberg)
'$INCLUDE:'palette.bi'
'$INCLUDE:'dither.bi'
...
SUB ApplyFloydSteinbergDither (img AS LONG, useSRGB AS _BYTE, serpentine AS _BYTE)
    ...
    ' Row buffers for error (r,g,b)
    DIM er#(0 TO w) AS DOUBLE, eg#(0 TO w) AS DOUBLE, eb#(0 TO w) AS DOUBLE
    DIM ner#(0 TO w) AS DOUBLE, neg#(0 TO w) AS DOUBLE, neb#(0 TO w) AS DOUBLE
