' Dithering Test with Graphics Window
$CONSOLE
_CONSOLE ON
...
PRINT "Controls: SPACE = next dither method, ESC = exit"
...
SUB CreateTestPalette
    ...
    PRINT "Palette created with"; PalCount; "colors"
END SUB
...
FUNCTION GetMethodName$(method AS INTEGER)
    SELECT CASE method
        CASE 0
            GetMethodName$ = "None (Quantize Only)"
        CASE 1
            GetMethodName$ = "Ordered Dither 2x2"
        CASE 2
            GetMethodName$ = "Floyd-Steinberg"
        CASE ELSE
