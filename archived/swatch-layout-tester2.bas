...
DIM SHARED AS LONG CANVAS, swatches
DIM SHARED AS INTEGER w, h, c, rows, cols, cell_w, cell_h, i, row, col, guide_opac
DIM SHARED AS INTEGER off_x, off_y, max_c, min_c, shortfall
...
DIM SHARED AS SINGLE aspect_ratio, num_rows, num_cells_per_row, num_cols
DIM SHARED AS SINGLE x1, y1, x2, y2
RANDOMIZE TIMER
...
SUB draw_output
    ...
    'new layout test
    aspect_ratio = w \ h
    num_cells_per_row = SQR(c * aspect_ratio)
    num_rows = c / num_cells_per_row
    num_cols = num_cells_per_row
    rows = num_rows
    cols = num_cols
    cell_w = w / cols
    cell_h = h / rows 
