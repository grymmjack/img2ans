...
DIM SHARED AS INTEGER rnd_r_off, rnd_g_off, rnd_b_off, small_off, large_off
DIM SHARED AS INTEGER max_r_off, max_g_off, max_b_off
DIM SHARED AS INTEGER min_r_off, min_g_off, min_b_off
    ...
    IF k$ <> "" THEN
        SELECT CASE k$
            ...
            CASE "'"
                ...
                draw_output
            CASE "R"
                rnd_r_off = rnd_r_off + small_off
            CASE "r"
                rnd_r_off = rnd_r_off - small_off
            CASE "G"
                rnd_g_off = rnd_g_off + small_off
            CASE "g"
                rnd_g_off = rnd_g_off - small_off
            CASE "B"
                rnd_b_off = rnd_b_off + small_off
            CASE "b"
                rnd_b_off = rnd_b_off - small_off
            CASE "="
                rnd_r_off = rnd_r_off + small_off
            ...
            CASE "_"
                ...
                rnd_b_off = rnd_b_off - small_off
        END SELECT
        IF rnd_r_off >= max_r_off THEN rnd_r_off = max_r_off
        IF rnd_g_off >= max_g_off THEN rnd_g_off = max_g_off
        IF rnd_b_off >= max_b_off THEN rnd_b_off = max_b_off
        IF rnd_r_off <= min_r_off THEN rnd_r_off = min_r_off
        IF rnd_g_off <= min_g_off THEN rnd_g_off = min_g_off
        IF rnd_b_off <= min_b_off THEN rnd_b_off = min_b_off
        draw_output
    END IF
LOOP
...
SUB draw_output
    ...
    PRINT "Offsets R: " + _TRIM$(STR$(rnd_r_off)), "G: " + _TRIM$(STR$(rnd_g_off)), "B: " + _TRIM$(STR$(rnd_b_off)),
    ...
    PRINT "Adjust RGB: - / = by " + _TRIM$(STR$(small_off)),
    PRINT "Adjust RGB: _ / + by " + _TRIM$(STR$(large_off))
    PRINT "Adjust [rR]ed [gG]reen [bB]lue by " + _TRIM$(STR$(small_off))
    ...
    FOR row=0 TO rows - 1
        FOR col=0 to cols - 1
            ...
            LINE (x1, y1)-(x2, y2), _RGB32(INT(RND*255)+rnd_r_off, INT(RND*255)+rnd_g_off, INT(RND*255)+rnd_b_off), BF
